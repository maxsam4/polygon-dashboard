-- Migration: Add min_priority_fee_avg to continuous aggregates
-- (block-count-weighted rollup of per-block MIN priority fee, for /stats min-fee tiles)
-- Note: This drops and recreates the continuous aggregates (data is recomputed from blocks table)
...
DROP MATERIALIZED VIEW IF EXISTS blocks_1hour_agg CASCADE;
DROP MATERIALIZED VIEW IF EXISTS blocks_1min_agg CASCADE;

-- Recreate 1-minute aggregate with min_priority_fee_avg
...
SELECT
  ...
  MAX(tps) AS tps_max,
  MAX(mgas_per_sec) AS mgas_per_sec_max,
  AVG(median_priority_fee_gwei) AS median_priority_fee_avg,
  AVG(min_priority_fee_gwei) AS min_priority_fee_avg
FROM blocks
GROUP BY bucket
WITH NO DATA;

-- Recreate 1-hour aggregate with min_priority_fee_avg
