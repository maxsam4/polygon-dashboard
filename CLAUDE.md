...
- **Per-gas fee averages**: `fees.avgBaseFeeGwei` / `avgMedianPriorityFeeGwei` / `avgMinPriorityFeeGwei` / `avgTotalFeeGwei` — block-count-weighted from `base_fee_avg` / `median_priority_fee_avg` (added 20260718_150000) / `min_priority_fee_avg` (added 20260718_170000) / `total_gas_price_avg` aggregate columns. UI splits Revenue (chainwide + producer totals) from Fees (averages + min-fee transfer tiles priced at base + min priority, gas units in `TRANSFER_GAS_UNITS`).
