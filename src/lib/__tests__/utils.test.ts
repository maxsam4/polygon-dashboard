...
describe('getGasUtilizationColor', () => {
  it('returns muted green for optimal range (55-75%)', () => {
    expect(getGasUtilizationColor(55)).toBe('bg-success/40');
    expect(getGasUtilizationColor(65)).toBe('bg-success/40');
    expect(getGasUtilizationColor(75)).toBe('bg-success/40');
  });

  it('returns muted yellow for suboptimal range (15-55% or 75-85%)', () => {
    expect(getGasUtilizationColor(15)).toBe('bg-warning/40');
    expect(getGasUtilizationColor(54)).toBe('bg-warning/40');
    expect(getGasUtilizationColor(76)).toBe('bg-warning/40');
    expect(getGasUtilizationColor(85)).toBe('bg-warning/40');
  });

  it('returns muted red for extreme values (<15% or >85%)', () => {
    expect(getGasUtilizationColor(0)).toBe('bg-danger/40');
    expect(getGasUtilizationColor(14)).toBe('bg-danger/40');
    expect(getGasUtilizationColor(86)).toBe('bg-danger/40');
    expect(getGasUtilizationColor(100)).toBe('bg-danger/40');
  });
});
