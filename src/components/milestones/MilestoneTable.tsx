...
export function MilestoneTable({ milestones, title = 'Milestones' }: Props) {
  return (
    <div className="glass-card-solid rounded-xl overflow-hidden relative">
      ...
      <div className="p-4 border-b border-accent/10 dark:border-accent/20">
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-surface dark:bg-surface-elevated/50">
            <tr>
              <th className="px-3 py-2 text-left text-muted font-medium">ID</th>
              <th className="px-3 py-2 text-right text-muted font-medium">From Block</th>
              <th className="px-3 py-2 text-right text-muted font-medium">To Block</th>
              <th className="px-3 py-2 text-right text-muted font-medium">Expected</th>
              <th className="px-3 py-2 text-right text-muted font-medium">In DB</th>
              <th className="px-3 py-2 text-right text-muted font-medium">Coverage</th>
              <th className="px-3 py-2 text-left text-muted font-medium">Proposer</th>
              <th className="px-3 py-2 text-left text-muted font-medium">Time</th>
              <th className="px-3 py-2 text-right text-muted font-medium">Avg Finality</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-accent/10 dark:divide-accent/15">
            {milestones.map((m) => {
              ...
              return (
                <tr key={m.milestoneId} className="hover:bg-surface-hover transition-colors">
                  <td className="px-3 py-2 font-mono text-xs text-accent">{m.milestoneId}</td>
                  ...
                  <td className="px-3 py-2 text-xs text-muted" title={m.timestamp}>
